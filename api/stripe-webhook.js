...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (event.type === "checkout.session.completed") {
      ...
      console.log("➡️ checkout.session.completed session:", session && session.id);
      ...
      if (session) {
        ...
        if (!userId) {
          console.warn(
            "checkout.session.completed without user_id metadata; session id:",
            session.id
          );
        } else {
          console.log("Updating user_plans for user:", userId, "email:", email);
